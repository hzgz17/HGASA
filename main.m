...
for num_file=start_num:NumFile
    ...
    for run=1:15 % run time
        ...
        while T>=Te
            for L=1:50  %max iterations for generating new solution at a temperature
                population_cross=cross_muta(population,distance,timewindow,servicetime,quality,depart,syn_points,worktime,num_caregiver,num_patient,num_syn,lunch_points,T);
                value=object(population_cross,distance,depart,lunch_points);
                [offspring_value,offspring_best_index]=min(value);
                if offspring_value<bestoutputs(j)
                    bestoutputs(j+1)=offspring_value;
                    bestroute(j+1,:)=population_cross(offspring_best_index,:);
                    count(j)=1;
                else
